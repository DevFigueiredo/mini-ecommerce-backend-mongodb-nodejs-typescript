...
let ListOrderController = class ListOrderController {
    constructor({ listOrderUseCase }) {
        this.listOrderUseCase = listOrderUseCase;
    }
    async execute(request, response) {
        const params = { status: request.params.status, customerId: request.customerId };
        const order = await this.listOrderUseCase.execute({ params });
        return response.status(http_status_helper_1.HttpStatusHelper.OK).json(order);
    }
};
__decorate([
    (0, awilix_express_1.GET)(),
    (0, awilix_express_1.route)('/:status/list/'),
